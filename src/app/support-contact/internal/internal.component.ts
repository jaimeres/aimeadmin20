import { Component, OnInit, signal } from '@angular/core';
import { ConfirmationService, MenuItem } from 'primeng/api';
import { CrudPageShellComponent } from '../../shared/crud-page-shell.component';
import { CRUD } from '../../utils/crud.class';
import { SupportContactService } from '../services/support-contact.service';

@Component({
  selector: 'app-internal',
  imports: [CrudPageShellComponent],
  templateUrl: './internal.component.html',
  styleUrl: './internal.component.scss',
  standalone: true,
  providers: [ConfirmationService]
})
export class InternalComponent extends CRUD implements OnInit {

  public override openNewMenu = signal<MenuItem[]>([{
    label: 'Contacto interno',
    command: () => this.openNew({ pos: 'support-contact-internal' })
  }]);

  public override getMenu = signal<MenuItem[]>([{
    label: 'Internos',
    command: () => this.getAll({ pos: 'support-contact-internal' })
  }]);

  constructor(crudS: SupportContactService) {
    super(crudS, 'support-contact-internal');
  }

  ngOnInit(): void {
    this.typeDefault = 'support-contact-internal';
    this.app[this.typeDefault] = 'support-contact/internal';
    this.module[this.typeDefault] = 'SC';
    this.initCRUD();
  }

}
