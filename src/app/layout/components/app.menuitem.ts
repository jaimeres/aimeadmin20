...
@Component({
  ...
  template: `
    <ng-container>
      <div *ngIf="root && item.visible !== false && !item.routerLink && !item.items" class="layout-menuitem-root-text">
        <span>{{ item.label }}</span>
        <i class="layout-menuitem-root-icon pi pi-fw pi-ellipsis-h"></i>
      </div>
      <a
        *ngIf="(!item.routerLink || item.items) && item.visible !== false && !(root && !item.routerLink && !item.items)"
        [attr.href]="item.url"
